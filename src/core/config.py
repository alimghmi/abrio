...
class Settings(BaseSettings):
    ...
    worker_task_soft_time_limit_seconds: int = Field(gt=0, default=30)
    worker_task_time_limit_seconds: int = Field(gt=0, default=60)
    ...
    def validate_worker_time_limits(self) -> Self:
        if self.worker_task_soft_time_limit_seconds >= self.worker_task_time_limit_seconds:
            raise ValueError(
                "worker_task_soft_time_limit_seconds must be less than "
                "worker_task_time_limit_seconds so the soft limit can roll back cleanly "
