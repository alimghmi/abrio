...
def test_users_balance_endpoint_flow(db_session_factory: SessionFactory) -> None:
    with db_session_factory() as session:
        ...
        assert created.balance.credits == 0
        assert created.balance.reserved_credits == 0
    ...
    with db_session_factory() as session:
        ...
        assert fetched.balance.available_credits == 0

    with db_session_factory() as session:
        topped_up = asyncio.run(
        ...
        assert topped_up.credits == 25
        assert topped_up.available_credits == 25

    with db_session_factory() as session:
        zeroed = asyncio.run(user_routes.zero_user_balance(user_id, BalanceUseCase(session)))

        assert zeroed.user_id == user_id
        assert zeroed.credits == 0
        assert zeroed.reserved_credits == 0
        assert zeroed.available_credits == 0
...
def test_get_user_endpoint_raises_for_missing_user(db_session_factory: SessionFactory) -> None:
    with db_session_factory() as session, pytest.raises(UserNotFoundError) as exc_info:
        asyncio.run(user_routes.get_user(999, UserUseCase(session)))
...
def test_zero_endpoint_raises_for_missing_user(db_session_factory: SessionFactory) -> None:
    with db_session_factory() as session, pytest.raises(UserNotFoundError) as exc_info:
        asyncio.run(user_routes.zero_user_balance(999, BalanceUseCase(session)))
